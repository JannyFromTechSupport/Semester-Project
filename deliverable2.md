...
| (b) ESP32 (MQ-5) ↔ ESP32 (DHT22) via UART | — | Working |
| (c) ESP32 (DHT22) → Relay → ESP32 (MQ-5) | Attempted | Working |
...
## Architecture (b): ESP32 (MQ-5/MQ2) ↔ ESP32 (DHT22) via UART
...
The team ran the simulation twice, once with each `NODE` value, to verify that both roles functioned correctly with their respective sensors, confirming the wiring and individual logic for each node was correct, even though true simultaneous dual-MCU execution isn't possible on Wokwi's web platform.
...
## Architecture (c): ESP32 (DHT22) → Relay → ESP32 (MQ-5/MQ2)
...
This occurred during the flashing stage (after "Hard resetting via RTS pin..."), before any application code could run meaning the failure was at the upload/communication level, not a sensor-reading issue.
...
- Replaced the lab-provided USB cable with the team's own USB cable, after determining the original cable was non-functional.
